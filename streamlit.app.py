...
def play_sam_intro():
    ...
    if "sam_intro_needs_playing" in st.session_state and st.session_state.sam_intro_needs_playing:
        try:
            ...
            if sam_messages:
                log.info("Playing Sam's introduction message")
                ...
                log.info("Sam's introduction completed")
        except Exception as e:
...
def process_user_query(text_client, speech_client, user_query):
    ...
    if not st.session_state.sam_active and not st.session_state.debrief_active:
        ...
        if re.search(ready_pattern, user_query.lower()):
            ...
            # TWO-STAGE TRANSITION with improved timing
            # First stage: Add Noa's transition message
            ...
            # Only play Noa's transition message in this first stage
            try:
                log.info("Playing Noa's transition message")
                ...
                wait_time = estimated_seconds + 2.5  # Minimum 2.5 second buffer after estimated completion
                log.info(f"Waiting {wait_time} seconds for Noa's audio to complete")
                
                # Wait for audio to complete with the buffer
                time.sleep(wait_time)
                
                # Make absolute sure any audio has completely stopped
                stop_current_audio()
                time.sleep(1.0)  # Additional pause after stopping audio
                
                log.info("Noa's transition message complete")
                
                # Now prepare for second stage - set up a flag to add Sam's message on next cycle
                st.session_state.ready_for_sam_intro = True
                
                # Rerun the app to update the UI and proceed to next stage
                st.rerun()
                return  # Skip further processing
                
            except Exception as e:
                log.exception(f"Error playing transition audio: {e}")
                # Fall back to direct transition if audio fails
                st.session_state.sam_active = True
                st.rerun()
                return
                
    # Second stage of transition - add Sam's message after Noa's audio completes
    if "ready_for_sam_intro" in st.session_state and st.session_state.ready_for_sam_intro:
        log.info("Starting second stage of transition - Sam's introduction")
        
        # Clear the flag
        st.session_state.ready_for_sam_intro = False
        ...
        # Schedule Sam's intro audio to play on the next cycle
        st.session_state.sam_intro_needs_playing = True
        
        # Force a rerun to update the UI for Sam's appearance
        st.rerun()
        return  # Skip further processing
...
def init_session():
    ...
    # New state variables for improved transition timing
    if "ready_for_sam_intro" not in st.session_state:
        st.session_state.ready_for_sam_intro = False
